...
window.MediaPreviewScreenClass = class MediaPreviewScreen {
     ...
     * OPTIMIZED: Account for AdMob banner row at bottom (50-60px)
     */
    applyOverlaySizing() {
        ...
        // AdMob banner height: ~50px standard banner + padding
        const bannerHeight = 60;
        
        if (isLandscape) {
            // Landscape mode: Optimized for maximum size above AdMob banner
            // Calculate available height: full viewport minus banner (60px) minus minimal margins (25px)
            const availableHeight = `calc(100vh - ${bannerHeight + 25}px)`;
            
            this.mediaOverlay.style.cssText = `
                position: fixed;
                top: 50%;
                left: 50%;
                transform: translate(-50%, calc(-50% - ${bannerHeight / 2}px));
                width: 75vw;
                max-width: 800px;
            ...
            // Portrait mode: Optimized to stay within screen bounds
            // Reduced width from 90vw to 85vw to prevent right-side overflow
            const availableHeight = `calc(100vh - ${bannerHeight + 40}px)`;
    ...
    async createMediaElement() {
        ...
        if (this.mediaType === 'youtube') {
            ...
            const isLandscape = window.innerWidth > window.innerHeight;
            const containerHeight = isLandscape ? '280px' : '320px';
    ...
    async show() {
        ...
        this.render();
        console.log(`🎵 Media preview screen shown with overlay`);
    }
