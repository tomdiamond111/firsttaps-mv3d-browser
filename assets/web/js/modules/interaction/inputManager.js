...
(function() {
    ...
    class InputManager {
        ...
        onPointerDown(event) {
            ...
            // DON'T preventDefault for mouse - let native click events work
        ...
        onPointerUp(event) {
            ...
            // CRITICAL FIX: Pointer events block native click events in browsers
            // Manually trigger click handling for desktop quick taps
            const tapDuration = Date.now() - this.stateManager.pointerDownStart;
            const isQuickTap = tapDuration < this.longPressDuration;
            ...
            if (isQuickTap) {
                ...
                if (intersects.length > 0) {
                    ...
                    const isDoubleTap = timeSinceLastTap < this.doubleTapThreshold && this.lastTapObject === object;
                    
                    if (isDoubleTap) {
                        console.log('⚡⚡ DOUBLE TAP detected on:', object.userData.fileName || object.userData.furnitureName);
                        this.lastTapTime = 0; // Reset to prevent triple tap detection
                        this.lastTapObject = null;
                        
                        // Handle double tap - open URL in browser
                        this.handleBrowserDoubleClick(object);
                    } else {
                        console.log('⚡ Quick tap detected - manually triggering click handler');
                        this.lastTapTime = now;
                        this.lastTapObject = object;
                        
                        console.log('🔍 CLICK: Object detected:', object.userData.fileName || object.userData.appName || object.userData.furnitureName);
                    ...
                    console.log('🔍 CLICK: Background click');
        ...
        /**
         * Browser-specific double click handler - opens URLs directly
         */
        handleBrowserDoubleClick(object) {
            console.log('🌐 Browser double-click handler for:', object.userData.fileName || object.name);
            ...
            } else if (object.userData.fileData && object.userData.fileData.url) {
                url = object.userData.fileData.url;
            } else if (object.userData.fileData && object.userData.fileData.thumbnailUrl) {
                url = object.userData.fileData.thumbnailUrl;
            }
            ...
            if (url) {
                ...
                console.warn('🌐 No URL found for object');
                // Still try to call the original handler as fallback
                this.onObjectDoubleClick(object);
            }
        }
