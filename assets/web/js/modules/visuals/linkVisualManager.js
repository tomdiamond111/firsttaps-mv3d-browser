    ...
    class LinkVisualManager {
        ...
        async addLogoToTopSurface(linkObject, linkData, category) {
            try {
                ...
                // Special handling for TikTok videos - try to get video thumbnail
                if (domain === 'tiktok.com') {
                    if (DEBUG) console.log(`🎵 [LOGO] ✅ Detected TikTok link, attempting thumbnail fetch for: ${url}`);
                    const thumbnailApplied = await this.tryTikTokThumbnail(linkObject, url, category, domain);
                    if (DEBUG) console.log(`🎵 [LOGO] TikTok thumbnail result: ${thumbnailApplied}`);
                    if (thumbnailApplied) {
                        return; // Successfully applied TikTok thumbnail
                    }
                    // If thumbnail fails, fall through to text logo
                    if (DEBUG) console.log(`🎵 [LOGO] TikTok thumbnail failed, using text logo fallback`);
                }
