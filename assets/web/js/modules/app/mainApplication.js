    ...
    class WindowWorldApp {
        ...
        setupGlobalReferences() {
            ...
            window.triggerFurnitureCreation = async () => {
                ...
                // BROWSER VERSION: Initialize remote config fetcher BEFORE furniture spawning
                if (window.browserRecommendationsFetcher) {
                    console.log('🌐 Initializing browser recommendations from GitHub Pages...');
                    try {
                        await window.browserRecommendationsFetcher.initialize();
                        console.log('✅ Browser recommendations initialized');
                    } catch (error) {
                        console.warn('⚠️ Failed to initialize browser recommendations:', error);
                        console.log('   Will fall back to hardcoded content');
                    }
                } else {
                    console.warn('⚠️ BrowserRecommendationsFetcher not available');
                }
                ...
                console.log(`🪑 Status check: firstInstall=${isFirstInstall}, hasDemoContent=${hasDemoContent}`);
                
                if (isFirstInstall) {
                    // FIRST INSTALL: Create furniture + demo content
                    // This only runs when BOTH marker is false AND furniture count is 0
                    ...
                    // Only runs if marker exists but demo content missing
            ...
            // Initialize Browser Menu Handler (comprehensive menu system for browser version)
            console.log('🌐 EXPLICIT: Checking for BrowserMenuHandler...');
            if (window.BrowserMenuHandler) {
                console.log('🌐 EXPLICIT: Creating BrowserMenuHandler instance...');
                this.browserMenuHandler = new window.BrowserMenuHandler(this);
                console.log('🌐 EXPLICIT: BrowserMenuHandler initialized successfully');
            } else {
                console.warn('🌐 EXPLICIT: BrowserMenuHandler not available');
            }
        ...
        startAnimationLoop() {
            const animate = () => {
                ...
                // Update entity/gaming system
                if (this.svgEntityManager && typeof this.svgEntityManager.update === 'function') {
                    this.svgEntityManager.update();
                }
