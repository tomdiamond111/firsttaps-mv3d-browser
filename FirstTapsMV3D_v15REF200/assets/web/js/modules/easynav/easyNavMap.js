...
(function() {
    ...
    class EasyNavMap {
        ...
        initialize() {
            ...
            // Calculate map size (approximately 23% of screen - 30% smaller than before)
        ...
        drawFurnitureShape(obj, mapCoords) {
            ...
            // Set white color for furniture
            this.ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
            this.ctx.strokeStyle = 'rgba(255, 255, 255, 1)';
            this.ctx.lineWidth = 1;
            
            // Get furniture type for shape rendering
            const furnitureType = obj.userData?.furnitureType || '';
            
            // Draw shape based on furniture type
            switch (furnitureType) {
                case 'amphitheatre':
                    this.drawAmphitheatreShape(mapCoords, mapWidth, mapDepth);
                    break;
                    
                case 'stage_small':
                case 'stage_large':
                    this.drawStageShape(mapCoords, mapWidth, mapDepth);
                    break;
                    
                case 'riser':
                    this.drawRiserShape(mapCoords, mapWidth, mapDepth);
                    break;
                    
                case 'bookshelf':
                    this.drawBookshelfShape(mapCoords, mapWidth, mapDepth);
                    break;
                    
                case 'gallery_wall':
                default:
                    // Gallery wall and other furniture: simple rectangle
                    this.drawRectangleShape(mapCoords, mapWidth, mapDepth);
                    break;
            }
        }
        
        /**
         * Draw rectangle shape (used for gallery wall and default)
         */
        drawRectangleShape(mapCoords, mapWidth, mapDepth) {
            const halfWidth = mapWidth / 2;
            const halfDepth = mapDepth / 2;
            
            this.ctx.fillRect(
                mapCoords.x - halfWidth,
                mapCoords.y - halfDepth,
                mapWidth,
                mapDepth
            );
        ...
        /**
         * Draw amphitheatre as semicircle
         */
        drawAmphitheatreShape(mapCoords, mapWidth, mapDepth) {
            const radius = Math.max(mapWidth, mapDepth) / 2;
            
            this.ctx.beginPath();
            // Draw arc from right to left (semicircle opening downward)
            this.ctx.arc(mapCoords.x, mapCoords.y, radius, 0, Math.PI);
            this.ctx.closePath();
            this.ctx.fill();
            this.ctx.stroke();
        }
        ...
        /**
         * Draw stage as trapezoid (wider at front)
         */
        drawStageShape(mapCoords, mapWidth, mapDepth) {
            const halfDepth = mapDepth / 2;
            const backWidth = mapWidth * 0.6;
            const frontWidth = mapWidth;
            
            this.ctx.beginPath();
            // Back left
            this.ctx.moveTo(mapCoords.x - backWidth / 2, mapCoords.y - halfDepth);
            // Back right
            this.ctx.lineTo(mapCoords.x + backWidth / 2, mapCoords.y - halfDepth);
            // Front right
            this.ctx.lineTo(mapCoords.x + frontWidth / 2, mapCoords.y + halfDepth);
            // Front left
            this.ctx.lineTo(mapCoords.x - frontWidth / 2, mapCoords.y + halfDepth);
            this.ctx.closePath();
            this.ctx.fill();
        ...
        /**
         * Draw riser as stepped rectangles
         */
        drawRiserShape(mapCoords, mapWidth, mapDepth) {
            const numSteps = 3;
            const stepDepth = mapDepth / numSteps;
            const stepWidthDecrement = mapWidth / (numSteps * 3);
            
            for (let i = 0; i < numSteps; i++) {
                const stepWidth = mapWidth - (i * stepWidthDecrement);
                const stepY = mapCoords.y - mapDepth / 2 + (i * stepDepth);
                
                this.ctx.fillRect(
                    mapCoords.x - stepWidth / 2,
                    stepY,
                    stepWidth,
                    stepDepth
                );
        ...
        /**
         * Draw bookshelf as rectangle with vertical dividers
         */
        drawBookshelfShape(mapCoords, mapWidth, mapDepth) {
            const halfWidth = mapWidth / 2;
            const halfDepth = mapDepth / 2;
            ...
            // Draw vertical dividers (shelves)
            const numDividers = 3;
            for (let i = 1; i < numDividers; i++) {
                const x = mapCoords.x - halfWidth + (mapWidth * i / numDividers);
                this.ctx.beginPath();
                this.ctx.moveTo(x, mapCoords.y - halfDepth);
                this.ctx.lineTo(x, mapCoords.y + halfDepth);
                this.ctx.stroke();
            }
        }
