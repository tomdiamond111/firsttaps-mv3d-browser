...
class URLProcessor {
    ...
    async processYouTubeURL(url, serviceData) {
        ...
        try {
            ...
            // Try to find a working thumbnail with fallback (prevents 404 errors in console)
            const thumbnailUrl = await this.findWorkingYouTubeThumbnail(videoId);
            ...
            const isLikelyDeleted = metadataFetchFailed && !thumbnailUrl;
    ...
    /**
     * Find a working YouTube thumbnail URL with fallback to lower qualities
     * Prevents 404 errors by testing thumbnails before returning URL
     * @param {string} videoId - YouTube video ID
     * @returns {Promise<string|null>} - Working thumbnail URL or null
     */
    async findWorkingYouTubeThumbnail(videoId) {
        // Try thumbnail qualities in order of preference
        // hqdefault is first because it's most reliable (unlike maxresdefault which often 404s)
    ...
    async processTikTokURL(url, serviceData) {
        ...
        try {
            ...
            // NON-BLOCKING: Start metadata fetch in background (don't await)
            // This prevents TikTok metadata from blocking furniture population (3-14s delays)
            console.log('🎵 [TIKTOK] Starting metadata fetch in background (non-blocking)...');
            this.requestTikTokMetadataFromFlutter(url).then(bridgeMetadata => {
                if (bridgeMetadata) {
                    console.log('🎵 [TIKTOK] ✅ Background fetch got metadata:', bridgeMetadata);
                    // Metadata will be handled by linkVisualManager's branding system
                } else {
                    console.log('🎵 [TIKTOK] Background fetch returned null');
                }
            }).catch(error => {
                console.warn('🎵 [TIKTOK] Background fetch failed:', error);
            });
            
            // Return immediately with default values (object creates instantly)
            // Branding/thumbnails will be applied later when metadata arrives
    ...
    async processInstagramURL(url, serviceData) {
        ...
        try {
            ...
            // NON-BLOCKING: Start metadata fetch in background (don't await)
            // This prevents Instagram metadata from blocking furniture population (2-5s delays)
            console.log('📸 [INSTAGRAM] Starting metadata fetch in background (non-blocking)...');
            this.requestInstagramMetadataFromFlutter(url).then(bridgeMetadata => {
                if (bridgeMetadata) {
                    console.log('📸 [INSTAGRAM] ✅ Background fetch got metadata:', bridgeMetadata);
                    // Metadata will be handled by linkVisualManager's branding system
                } else {
                    console.log('📸 [INSTAGRAM] Background fetch returned null');
                }
            }).catch(error => {
                console.warn('📸 [INSTAGRAM] Background fetch failed:', error);
            });
    ...
    async processVimeoURL(url, serviceData) {
        ...
        try {
            ...
            // NON-BLOCKING: Start metadata fetch in background (don't await)
            // Prevents Vimeo metadata from blocking furniture population
            console.log('🎬 [VIMEO] Starting metadata fetch in background (non-blocking)...');
            this.fetchVimeoOEmbed(url).then(oEmbedResponse => {
                if (oEmbedResponse) {
                    console.log('🎬 [VIMEO] ✅ Background fetch got metadata:', oEmbedResponse.title);
                    // Metadata will be handled by linkVisualManager's branding system
                }
            }).catch(error => {
                console.warn('🎬 [VIMEO] Background fetch failed:', error);
            });
    ...
    async processDailymotionURL(url, serviceData) {
        ...
        try {
            ...
            // NON-BLOCKING: Start metadata fetch in background (don't await)
            // Prevents Dailymotion metadata from blocking furniture population
            console.log('🎬 [DAILYMOTION] Starting metadata fetch in background (non-blocking)...');
            this.fetchDailymotionOEmbed(url).then(oEmbedResponse => {
                if (oEmbedResponse) {
                    console.log('🎬 [DAILYMOTION] ✅ Background fetch got metadata:', oEmbedResponse.title);
                    // Metadata will be handled by linkVisualManager's branding system
                }
            }).catch(error => {
                console.warn('🎬 [DAILYMOTION] Background fetch failed:', error);
            });
    ...
    requestInstagramMetadataFromFlutter(url) {
        return new Promise((resolve) => {
            ...
            // Set timeout in case Flutter doesn't respond (reduced from 5s to 2s)
            const timeoutId = setTimeout(() => {
    ...
    async fetchTikTokOEmbed(url) {
        ...
        const maxRetries = 2; // Reduced from 3 to 2 to speed up first install
        const timeouts = [2000, 3000]; // Reduced from [6s,10s,15s] to [2s,3s] for faster loading
