...
(function() {
    ...
    class SpaceWorldTemplate extends BaseWorldTemplate {
        ...
        getHomeViewPosition() {
            // Staging area view for space world - adjust based on screen orientation
            const aspectRatio = window.innerWidth / window.innerHeight;
            const isLandscape = aspectRatio > 1.2; // Consider landscape if width > 1.2x height
            
            if (isLandscape) {
                // Landscape: closer view of staging area
                return { x: 0, y: 8, z: 20 }; // Closer staging area view for landscape
            } else {
                // Portrait: higher and further back for broader view
                return { x: 0, y: 10, z: 25 }; // Full staging area view for portrait
            }
        }
    ...
    class OceanWorldTemplate extends BaseWorldTemplate {
        ...
        getHomeViewPosition() {
            // Staging area view for ocean world - adjust based on screen orientation
            const aspectRatio = window.innerWidth / window.innerHeight;
            const isLandscape = aspectRatio > 1.2; // Consider landscape if width > 1.2x height
